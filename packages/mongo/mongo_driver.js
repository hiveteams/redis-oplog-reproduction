...
MongoConnection.prototype._dropCollection = function (collectionName, cb) {
  ...
  var refresh = function () {
    Meteor.refresh({
      collection: collectionName, id: null,
      dropCollection: true
    });
  };
  ...
  if (firstHandle) {
    ...
    var canUseOplog = _.all([
      function () {
        ...
        // that won't happen.
        const res = self._oplogHandle && !ordered &&
          !callbacks._testOnlyPollCallback;

        console.log(cursorDescription.collectionName, 'canUseOplog 1', res);
        if (!res) {
          // console.log(JSON.stringify(cursorDescription.selector));
          // console.log(JSON.stringify(cursorDescription.options));
